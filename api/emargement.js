...
export default async function handler(req, res) {
  ...
  // Charge formateur + session + état des émargements (token = ID de la session)
  async function loadContextFormateur(token) {
    const pageId = token.replace(/-/g, "");
    const session = await getPage(pageId);
    if (!session || session.object === "error") return { error: "Token invalide" };
    if ((session.parent?.database_id || "").replace(/-/g, "") !== SESSIONS_DB) {
      return { error: "Token invalide" };
    }

    const formateurIds = rel(session, "👩‍🏫 Formateur·rice");
    if (!formateurIds.length) {
      return { error: "Aucun formateur n'est associé à cette session" };
    }
    const formateur = await getPage(formateurIds[0]);
    if (!formateur) return { error: "Fiche formateur introuvable" };
    ...
    // Émargements formateur existants sur CETTE session
    const existing = await queryDB(EMARGEMENTS_DB, {
      filter: {
        and: [
          { property: "Formateur", relation: { contains: formateur.id } },
          { property: "Session", relation: { contains: session.id } },
        ],
      },
      page_size: 100,
    });
    ...
    return {
      ...
      participant: {
        id: formateur.id,
        nom: title(formateur, "Nom"),
        prenom: "",
        fonction: text(formateur, "Fonction"),
      },
      session: {
        id: session.id,
        titre: title(session, "Titre formation"),
  ...
  if (req.method === "GET") {
    ...
    const isFormateur = req.query.role === "formateur";
    try {
      const ctx = isFormateur ? await loadContextFormateur(token) : await loadContext(token);
  ...
  if (req.method === "POST") {
    try {
      const { token, creneau, signature, role } = req.body || {};
      ...
      const create = await fetch("https://api.notion.com/v1/pages", {
        ...
        body: JSON.stringify({
          ...
          properties: {
            "Identifiant": { title: [{ text: { content: `${ctx.participant.nom}${isFormateur ? " (formateur)" : ""} — ${ctx.session.code || ctx.session.titre} — ${creneau}` } }] },
            ...(isFormateur
              ? { "Formateur": { relation: [{ id: ctx.participant.id }] } }
              : { "Participant": { relation: [{ id: ctx.participant.id }] } }),
            "Signataire": { select: { name: isFormateur ? "Formateur" : "Stagiaire" } },
      ...
      if (signedCount >= total && !isFormateur) {
