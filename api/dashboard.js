...
export default async function handler(req, res) {
  ...
  try {
    ...
    const formateurIds = [...new Set(sessions.flatMap(s => rel(s, "👩‍🏫 Formateur·rice")))];
    const [participants, formations, entreprises, formateurs] = await Promise.all([
      fetchPages(participantIds), fetchPages(formationIds), fetchPages(entrepriseIds), fetchPages(formateurIds),
    ]);
    ...
    const signCount = {};
    const signCountFormateur = {};
    for (const e of emargements) {
      for (const pid of rel(e, "Participant")) signCount[pid] = (signCount[pid] || 0) + 1;
      if (rel(e, "Formateur").length) {
        for (const sid of rel(e, "Session")) signCountFormateur[sid] = (signCountFormateur[sid] || 0) + 1;
      }
    }
    ...
    const payload = sessions.map(s => {
      ...
      return {
        ...
        ressources: {
          ...
          supportFormateur: f ? files(f, "Support formateur") : [],
        ...
        formateur: (() => {
          const frId = rel(s, "👩‍🏫 Formateur·rice")[0];
          const fr = frId ? formateurs[frId] : null;
          if (!fr) return null;
          return {
            id: frId.replace(/-/g, ""),
            nom: title(fr, "Nom"),
            fonction: text(fr, "Fonction"),
            emargementsSignes: signCountFormateur[s.id] || 0,
          };
        })(),
