...
export default async function handler(req, res) {
  ...
  async function loadContext(token) {
    ...
    return {
      ...
      evalRecId: rel(participant, "📊 Évaluation")[0] || null,
  ...
  if (req.method === "GET" && req.query.mode === "resultats") {
    ...
    try {
      ...
      if (!ctx.dejaComplete || !ctx.evalRecId) {
        return res.status(404).json({ error: "Évaluation non complétée", nonComplete: true });
      }
      const evalPage = await getPage(ctx.evalRecId);
      ...
      let details = null;
      if (jsonPropId) {
        try { details = JSON.parse(await readLongText(ctx.evalRecId, jsonPropId)); } catch { details = null; }
      }
      const resumePropId = evalPage.properties?.["Réponses écrit"]?.id;
      const resume = (!details && resumePropId) ? await readLongText(ctx.evalRecId, resumePropId) : null;
